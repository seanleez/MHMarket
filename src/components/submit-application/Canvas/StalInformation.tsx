...
const StallInformation: React.FC<IStallInformation> = ({ position, stall }) => {
  ...
  const apply = () => {
    navigate(
      `/application/create?stall_code=${stall.code}&stall_type=${stall.stall_type}&stall_class=${stall.stall_class}&stall_area=${stall.area}&market_code=${stall.market_code}&market_type=${stall.market_type}&market_class=${stall.market_class}&floor_code=${stall.floor_code}`
    );
  };
    ...
    <Paper
      ...
      {stall?.is_updated_detail === true && (
        <Card variant="outlined" sx={{ padding: 1.5 }}>
          ...
          {stall?.lease_status === 0 && (
            <CardActions
              sx={{ justifyContent: 'flex-end', padding: '10px 0 0' }}>
